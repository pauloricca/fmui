...
const voiceCompare=$('#compare').onclick,voiceUndo=$('#undo').onclick;
const effectInitial=synth.effects.createState();
let effectState=structuredClone(effectInitial),effectHistory=[],effectCompare=false;
function effectShown(){return effectCompare?effectInitial:effectState;}
function effectRemember(){effectHistory.push(structuredClone(effectState));if(effectHistory.length>100)effectHistory.shift();}
...
function effectReadout(){const e=effectShown();$('#fx-name').textContent=effects.names[e.preset];$('#fx-summary').textContent=`${e.preset} / ${effects.names[e.preset]} · ${e.preset>=8?'SIZE':'TIME'} ${e.time} · BALANCE ${e.balance}${e.balance===0?' / OFF':''}`;$('#fx-description').textContent=e.preset>=8?'ROOM SIZE CONTROLS THE GATED REVERB.':[3,4,5,7].includes(e.preset)?'TIME CONTROLS THE DELAY / ECHO INTERVAL.':'TIME CONTROLS THE REVERB DURATION.';if(effectsVisible){$('#undo').disabled=effectCompare||!effectHistory.length;$('#compare').setAttribute('aria-pressed',String(effectCompare));}effectStatus('EFFECTS · LOCAL EDIT · NO MIDI CONNECTION');}
...
const undoEffects=()=>{if(!effectCompare&&effectHistory.length){effectState=effectHistory.pop();renderEffects();}};
$('#compare').onclick=()=>effectsVisible?compareEffects():voiceCompare();
$('#undo').onclick=()=>effectsVisible?undoEffects():voiceUndo();
for(let n=1;n<=16;n++){const option=document.createElement('option');option.value=n;option.textContent=n;$('#fx-channel').append(option);}
$('#fx-save').onclick=()=>{const bytes=effects.bulk(effectShown(),Number($('#fx-channel').value));const url=URL.createObjectURL(new Blob([bytes],{type:'application/octet-stream'}));const a=document.createElement('a');a.href=url;a.download=synth.shortName+'-effects.syx';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);effectStatus('EFFECTS SYSEX SAVED · NOT TRANSMITTED');};
