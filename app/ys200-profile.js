...
SENS:${op.EBS} ${op.KVS}  OUT LEVEL:${op.OUT}`,
  seed:voice=>voice.operators.forEach(op=>{op.AR=8+Math.floor(Math.random()*24);op.D1R=Math.floor(Math.random()*25);op.D1L=5+Math.floor(Math.random()*11)}),
  ...
  captions:{global:[['',1,3],['LOW FREQUENCY OSCILLATOR',3,9],['SENSITIVITY',9,14],['',14,17]],operator:[['OSCILLATOR',1,7],['ENVELOPE GENERATOR',7,13],['REV',13,14],['KBD SCALING',14,16],['OUT',16,17]]},
};
})();
