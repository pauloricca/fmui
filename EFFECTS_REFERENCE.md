...
Bulk export: F0 43 0n 7E 00 0D [ASCII "LM  8036EF"] [preset time balance] [checksum] F7. Identifier contains TWO spaces. Checksum negates the sum of all 13 payload bytes modulo 128. Total length 21 bytes. Export changes the edit-buffer data when loaded by compatible MIDI software; it is not a memory-store command. No transmission or audio preview is implemented.

The documented EFEDS request also returns ACED2, ACED and VCED; it must not be treated as an effects-only reply in a future MIDI receiver. VMEM effects reside at offsets 91–93. Voice ACED REV is a separate envelope pseudo-reverb parameter.

Effects have a separate local buffer and undo/compare history, retained across view switches. SAVE .SYX exports the displayed effects buffer; the existing voice JSON save remains voice-only. Reloading resets the prototype. Defaults are local demonstration values, not claimed factory values.
